...
def urlzone_crc(data):
    lookup_table = [0, 0x77073096, 0xEE0E612C, 0x990951BA,
...
if __name__=="__main__":
    ...
    parser.add_argument("-d", "--domain", help="initial domain")
    parser.add_argument("-v", "--version", help="version, 1 or 2", \
                            type=int, default=1)
    ...
    if version == 2:
        rsa_key = "BgIAAACkAABSU0ExAAQAAAEAAQBxQGcZ/br3h0ZKC2scS7WJ6Je4pOB" \
                    "5TOvOAe6mkPQ5zSruIlxkPpr9PLLGfuTEmAqVTBunRk0+ZpvnAXfHb" \
                    "TSYOD/bfmqtaE1pEeci8jjukAZZwCX+rWoh2O0ymX2v4rLbEqiC7ap" \
                    "bg+ughDvLEjkvkx2FBoHU55ALmfldAtPLnA=="
        rsa_data = base64.b64decode(rsa_key)
        crc = urlzone_crc(rsa_data)

    for i in range(2001):
        print(domain)
        domain = get_next_domain(domain, crc, version)
